...
let counterIds = getCounterIds(treeCounter);


function getCounterIds(tree) {
    // Initialize an empty array to store the IDs
    let ids = [];
  
    // Iterate over the branches of the tree
    for (let branch in tree) {
      // If the branch has a 'counters' property, iterate over its elements
      if (tree[branch].counters) {
        tree[branch].counters.forEach(counter => {
          // Add the counter's ID to the list
          ids.push(counter.id);
  
          // If the counter has child counters, recursively call the function to get their IDs
          if (counter.child) {
            ids = ids.concat(getCounterIds(counter.child));
          }
        });
      }
    }
  ...
  console.log(getCounterIds(treeCounter));
