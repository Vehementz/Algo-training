const jsonTree = {
  ...
  function addInfos(id, nom, localisation, bros ){
    ...
    if ( bros.length > 2) infos["layout"] = "hanging"
    infosData.push(infos)
  }
  
  /* function addInfosMore(id, nom, localisation, layout ){
    infosData.push({
      name: nom,
      description: localisation,
      id: id,
      title: "<a href='https://google.com' tabab", "autre": "http:erer" },
          arget='_blank'>ⓘ</a> <a href='https://billy.com'  target='_blank'>📄</a>",
      layout: "hanging"
    })
  } */
  ...
  jsonTree.counters.map(({child, id, localisation, nom}) => {
    addInfos(id, nom, localisation, jsonTree.counters)
    /* if(child.length > 0) {
