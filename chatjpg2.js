...
function treeCounterMapper(treeCounter) {
    const result = [];
  
    for (let key in treeCounter) {
      if (treeCounter.hasOwnProperty(key)) {
        const element = treeCounter[key];
  
        if (element.counters.length > 0) {
          element.counters.forEach((counter) => {
            const counterId = counter.id;
            result.push(counterId);
  
            if (counter.child.length > 0) {
              counter.child.forEach((childCounter) => {
                const childCounterId = childCounter.id;
                result.push([counterId, childCounterId]);
              });
            }
          });
        }
